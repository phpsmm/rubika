...
class Aghasocial_AI_Pages_Admin {
    public function __construct() {
        ...
        add_action('admin_post_aghasocial_ai_pages_generate_image', [$this, 'handle_manual_generate_image']);
    ...
    public function sanitize_settings($input) {
        ...
        aghasocial_ai_pages_log('settings_save', [
            'settings' => $settings,
        ], [
            'elementor_template_length' => isset($settings['elementor_template']) ? strlen((string) $settings['elementor_template']) : 0,
            'pack_template_length' => isset($settings['pack_template']) ? strlen((string) $settings['pack_template']) : 0,
        ], true);
        return $settings;
    }
    ...
    public function render_settings_page() {
        ...
        $image_url = add_query_arg('token', $token, plugins_url('cron/image.php', AGHASOCIAL_AI_PAGES_FILE));
        global $wpdb;
        ...
        <div class="wrap">
            ...
            <form method="post" action="options.php">
                ...
                <table class="form-table">
                    ...
                    <tr>
                        <th scope="row">Image Generation Mode</th>
                        <td>
                            <select name="<?php echo esc_attr(AGHASOCIAL_AI_PAGES_OPTION); ?>[image_generation_mode]">
                                <option value="inline" <?php selected($settings['image_generation_mode'], 'inline'); ?>>Inline (same request)</option>
                                <option value="defer" <?php selected($settings['image_generation_mode'], 'defer'); ?>>Deferred (image queue)</option>
                            </select>
                            <p class="description">Deferred mode queues image generation to avoid request timeouts.</p>
                        </td>
                    </tr>
                    <tr>
            ...
            <p>Image: <code><?php echo esc_html($image_url); ?></code></p>
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                <?php wp_nonce_field('aghasocial_ai_pages_manual'); ?>
                <input type="hidden" name="action" value="aghasocial_ai_pages_generate_image" />
                <?php submit_button('Generate 1 Image', 'secondary', 'submit', false); ?>
            </form>
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                <?php wp_nonce_field('aghasocial_ai_pages_manual'); ?>
    ...
    public function handle_manual_generate_image() {
        if (!current_user_can('manage_options')) {
            wp_die('Forbidden');
        }
        check_admin_referer('aghasocial_ai_pages_manual');
        ...
        $this->redirect_with_notice('Generate image result: ' . $result);
    }
    ...
    public function handle_overrides_save() {
        ...
        aghasocial_ai_pages_log('overrides_save', [
            'overrides' => $overrides,
        ], [
    ...
    public function handle_template_update() {
        ...
        aghasocial_ai_pages_log('template_update', [
            'template_type' => $template_type,
            'page_id' => $page_id,
            'page_template' => $page_template,
            'elementor_data' => $elementor_data,
        ], [
            'elementor_length' => strlen((string) $elementor_data),
        ], true);
    ...
    public function handle_template_build() {
        ...
        aghasocial_ai_pages_log('template_build', [
            'elementor_data' => $elementor_data,
        ], [
            'elementor_length' => strlen(wp_json_encode($elementor_data, JSON_UNESCAPED_UNICODE)),
        ], true);
    ...
    public function handle_template_rebuild() {
        ...
        $elementor_data = $this->build_template_from_ai();
        aghasocial_ai_pages_log('template_rebuild', [
            'page_id' => $page_id,
            'elementor_data' => $elementor_data,
        ], [
            'elementor_length' => strlen(wp_json_encode($elementor_data, JSON_UNESCAPED_UNICODE)),
    ...
    private function build_template_from_ai() {
        ...
        aghasocial_ai_pages_log('template_prompt', [
            'system' => $system,
            'prompt' => $prompt,
            'model' => $settings['template_builder_model'],
            'json_schema' => !empty($settings['template_use_json_schema']),
        ], [], true);
        $schema = [
