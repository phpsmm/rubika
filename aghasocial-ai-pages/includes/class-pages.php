...
class Aghasocial_AI_Pages_Pages {
    public function enqueue_missing_pages() {
        ...
        aghasocial_ai_pages_log('enqueue_missing_pages_start', [
            'settings' => $settings,
        ], [
        ...
        foreach ($categories as $category) {
            ...
            if (!$exists) {
                ...
                if (!$queued) {
                    ...
                    aghasocial_ai_pages_log('queue_generate_added', [
                        'payload' => $payload,
                    ], [
        ...
        foreach ($services as $service) {
            ...
            $services_by_category[(int) $service->cate_id] = true;
            ...
            if ($topic !== '') {
                ...
                if (!isset($topic_groups[$topic_key])) {
                    $topic_groups[$topic_key] = [
                        'topic' => $topic,
                        'services' => [],
                        'category_id' => (int) $service->cate_id,
                        'primary_name' => $service->name,
                    ];
                }
        ...
        foreach ($categories as $category) {
            ...
            if (!empty($services_by_category[(int) $category->id])) {
                continue;
            }
            $category_override = $category_overrides[(int) $category->id] ?? null;
            if (!$category_override) {
            ...
            $topic_override = '';
            if (!empty($category_override['topic'])) {
                $topic_override = $category_override['topic'];
            }
            $topic = $topic_override ?: $category->name;
            if ($topic === '') {
                continue;
            }
            $topic_key = aghasocial_ai_pages_normalize_title($topic);
        ...
        foreach ($groups as $normalized => $group_services) {
            ...
            if ($service_page) {
                ...
                if (!$queued) {
                    ...
                    aghasocial_ai_pages_log('queue_service_added', [
                        'payload' => $payload,
                    ], [
                        'queue_id' => $wpdb->insert_id,
                    ], true);
        ...
        foreach ($topic_groups as $topic_key => $group) {
            ...
            $primary = $group_services[0] ?? null;
            if (!$category_id && $primary) {
                $category_id = (int) $primary->cate_id;
            }
            if ($include_categories && $category_id && !in_array((int) $category_id, $include_categories, true)) {
                continue;
            }
            if ($exclude_categories && $category_id && in_array((int) $category_id, $exclude_categories, true)) {
                continue;
            }
            $category_override = $category_overrides[(int) $category_id] ?? null;
            if (!$category_override) {
                continue;
            }
            ...
            if ($primary && $excluded_services && in_array((int) $primary->id, $excluded_services, true)) {
                continue;
            }
            ...
            if ($primary) {
                ...
                foreach ($country_pages as $adjective) {
                    ...
                    if ($country_page) {
                        ...
                        $this->update_country_page((int) $country_page->page_id, (int) $category_id, $title);
                        continue;
                    }
                    $payload = [
                        'type' => 'country',
                    ...
                    if (!$queued) {
                        ...
                        aghasocial_ai_pages_log('queue_country_added', [
                            'payload' => $payload,
                        ], [
                            'queue_id' => $wpdb->insert_id,
                        ], true);
            ...
            $primary_name = $primary ? $primary->name : ($group['primary_name'] ?? $base_name);
            $has_country_term = $primary ? aghasocial_ai_pages_contains_country_terms($primary_name, $countries) : false;
            $ref_id = $primary ? (int) $primary->id : (int) $category_id;
            if (!$has_country_term) {
                foreach ($quantities as $quantity) {
                    ...
                    if (!$queued) {
                        ...
                        aghasocial_ai_pages_log('queue_quantity_added', [
                            'payload' => $payload,
                        ], [
                            'queue_id' => $wpdb->insert_id,
                        ], true);
            ...
            if (!empty($settings['enable_country_quantity'])) {
                ...
                $matched_country = $has_country_term ? aghasocial_ai_pages_detect_country($primary_name, $countries) : null;
                ...
                foreach ($country_list as $country) {
                    ...
                    foreach ($quantities as $quantity) {
                        ...
                        if (!$queued) {
                            ...
                            aghasocial_ai_pages_log('queue_country_quantity_added', [
                                'payload' => $payload,
                            ], [
                                'queue_id' => $wpdb->insert_id,
                            ], true);
        ...
        aghasocial_ai_pages_log('enqueue_missing_pages_done', [], [
            'time' => current_time('mysql'),
        ], true);
    }
    ...
    public function generate_one_image() {
        $settings = aghasocial_ai_pages_get_settings();
        ...
        global $wpdb;
        $queue_table = $wpdb->prefix . AGHASOCIAL_AI_PAGES_QUEUE_TABLE;
        $task = $wpdb->get_row("SELECT * FROM {$queue_table} WHERE type = 'image' AND status = 'pending' ORDER BY id ASC LIMIT 1");
        if (!$task) {
            return 'no_tasks';
        }
        ...
        if (!$post_id) {
            $wpdb->update($queue_table, [
                'status' => 'error',
                'last_error' => 'missing_post_id',
                'updated_at' => current_time('mysql'),
            ], ['id' => $task->id]);
            aghasocial_ai_pages_log('image_task_error', [
                'task_id' => $task->id,
                'payload' => $payload,
            ], [
        ...
        $elementor_data = $elementor_raw ? json_decode($elementor_raw, true) : [];
        if (!$elementor_data) {
            $wpdb->update($queue_table, [
                'status' => 'error',
                'last_error' => 'missing_elementor_data',
                'updated_at' => current_time('mysql'),
            ], ['id' => $task->id]);
            aghasocial_ai_pages_log('image_task_error', [
                'task_id' => $task->id,
    ...
    private function enqueue_image_task($post_id, $title, $service_name, $page_type, $category_id, $service_id) {
        global $wpdb;
        $queue_table = $wpdb->prefix . AGHASOCIAL_AI_PAGES_QUEUE_TABLE;
        $payload = [
            'post_id' => (int) $post_id,
        ...
        $payload_json = wp_json_encode($payload, JSON_UNESCAPED_UNICODE);
        $queued = $wpdb->get_var($wpdb->prepare(
            "SELECT id FROM {$queue_table} WHERE type = 'image' AND status = 'pending' AND payload = %s LIMIT 1",
            $payload_json
        ));
        if (!$queued) {
            $wpdb->insert($queue_table, [
            ...
            aghasocial_ai_pages_log('queue_image_added', [
                'payload' => $payload,
            ], [
                'queue_id' => $wpdb->insert_id,
            ], true);
    ...
    private function build_page($payload) {
        ...
        if ($type === 'quantity') {
            return $this->create_quantity_page($service_ids, $quantity, $country, $planned_title, (int) ($payload['category_id'] ?? 0));
        }
    ...
    private function create_category_page($category_id) {
        ...
        $placeholders = $this->build_ai_placeholders($title, $category->name, $category->description ?? '');
    ...
    private function create_service_page($service_ids) {
        ...
        $placeholders = $this->build_ai_placeholders($title, $service->name, $service->description ?? '');
    ...
    private function create_quantity_page($service_ids, $quantity, $country = null, $planned_title = null, $category_id = 0) {
        global $wpdb;
        $settings = aghasocial_ai_pages_get_settings();
        $service_id = (int) ($service_ids[0] ?? 0);
        $service = null;
        ...
        if (!$service) {
            ...
            $fallback_name = $category ? $category->name : '';
            $title = $planned_title ?: trim(sprintf('خرید %d %s %s', $quantity, $fallback_name, $country ? $country : ''));
            if ($title === '') {
                return new WP_Error('missing_service', 'Service not found');
            }
            $elementor_data = $this->build_kando_pack_group([], $quantity, $fallback_name);
            $placeholders = $this->build_ai_placeholders($title, $fallback_name, $category->description ?? '');
            return $this->create_elementor_page($title, '', $elementor_data, $placeholders, $fallback_name, (int) $category_id, 0, 'quantity');
        }

        $title = $planned_title ?: trim(sprintf('خرید %d %s %s', $quantity, $service->name, $country ? $country : ''));
        $pack_elements = $this->build_kando_pack_group($service_ids, $quantity, $service->name);
        $elementor_data = $pack_elements;
        $template = $settings['elementor_template'] ?? '';
        if ($template) {
            $decoded = $this->decode_elementor_json($template, 'create_quantity');
            if ($decoded) {
    ...
    private function update_quantity_page($page_id, $service_ids, $quantity, $country = null, $planned_title = null, $category_id = 0) {
        if (!$page_id) {
            return;
        }
        ...
        global $wpdb;
        $service_id = (int) ($service_ids[0] ?? 0);
        $service = $service_id ? $wpdb->get_row($wpdb->prepare("SELECT name FROM {$wpdb->prefix}samyar_services WHERE id = %d", $service_id)) : null;
        if (!$service) {
            if ($planned_title) {
                wp_update_post([
                    'ID' => $page_id,
                    'post_title' => $planned_title,
                ]);
            }
            return;
        }
    ...
    private function create_elementor_page($title, $content, $elementor_data, $placeholders = [], $service_name = '', $category_id = 0, $service_id = 0, $page_type = 'default') {
        ...
        if (is_wp_error($post_id)) {
            aghasocial_ai_pages_log('page_create_error', [
                'title' => $title,
                'slug' => $slug,
                'page_type' => $page_type,
                'category_id' => $category_id,
        ...
        $template = $settings['elementor_template'];
        if ($template) {
            $decoded = $this->decode_elementor_json($template, 'create_' . $page_type);
            if ($decoded) {
        ...
        if (!empty($elementor_data)) {
            ...
            $image_mode = $settings['image_generation_mode'] ?? 'inline';
            if (!empty($settings['enable_ai_images']) && $image_mode === 'inline') {
                $image_result = $this->attach_ai_images($elementor_data, $title, $service_name);
                $elementor_data = $image_result['data'];
                if (!empty($image_result['image'])) {
                    $image_placeholders = [
            ...
            aghasocial_ai_pages_log('elementor_data_empty', [
                'page_id' => $post_id,
                'page_type' => $page_type,
                'category_id' => $category_id,
                'service_id' => $service_id,
            ], [
                'template_length' => strlen((string) $template),
            ], true);
        }
    ...
    private function build_ai_placeholders($title, $service_name, $fallback_description = '') {
        $settings = aghasocial_ai_pages_get_settings();
        $fallback_description = $this->normalize_ai_text($fallback_description);
        if ($fallback_description === '') {
            $fallback_description = $service_name;
        }
        $fallback_content = $fallback_description;
        if ($fallback_content !== '' && strpos($fallback_content, '<') === false) {
            $fallback_content = '<p>' . $fallback_content . '</p>';
        }
        ...
        aghasocial_ai_pages_log('placeholders_prompt', [
    ...
    private function decode_elementor_json($json, $context) {
        ...
        if (!is_array($decoded)) {
            aghasocial_ai_pages_log('elementor_template_invalid', [
                'context' => $context,
                'length' => strlen($json),
            ], [
